...
class App extends React.Component {
    ...
    render() {
        ...
        return <div className="inu-example">
            ...
            <div className="inputs">
                <div className="row">
                    <div className="col"> <Input title="类型" select={select} value={edit.what} onChange={(v)=>edit.what=v}></Input> </div>
                    <div className="col"> <Input title="电影" select={this.state.movies} value={edit.mid} onChange={(v)=>edit.mid=v}></Input> </div>
                    <div className="col"> <Input title="公司" select={this.state.companys} value={edit.cid} onChange={(v)=>edit.cid=v}></Input> </div>
                    <div className="col"> <Input title="名称" value={edit.name} onChange={(v)=>edit.name=v} onClick={this.addLine.bind(this,edit,-1)}></Input> </div>
                </div>{
                this.state.data.params.map((item,i)=>{
                    return <div className="row">
                        <div className="col"> <Input title="类型" select={select} value={item.what} onChange={(v)=>item.what=v}></Input> </div>
                        <div className="col"> <Input title="电影" select={this.state.movies} value={item.mid} onChange={(v)=>item.mid=v}></Input> </div>
                        <div className="col"> <Input title="公司" select={this.state.companys} value={item.cid} onChange={(v)=>item.cid=v}></Input> </div>
                        <div className="col"> <Input title="名称" value={item.name} onChange={(v)=>item.name=v} onClick={this.addLine.bind(this,item,i)}></Input> </div>
                    </div>
                })
            }</div>
        </div>
