...
class App extends React.Component {
    constructor(props) {
        ...
        this.state = {
            head:[
                ...
                },"标题","原价","折扣价",
                {
                    title:"内容",
                    convert:(item,row)=>{
                        if(!item)return row['content'] = "[]"
                        return item.substring(0,10)
                    },
                    renderInput:(row,key,title)=>{
                        return <PaperEdit value={JSON.parse(row[key])} onChange={(v,c)=>{
                            console.log(c)
                            row[5] = c
        ...
        this.sql = new sql("paper",["id","title","price","promotion_price","content","question_num","category_id","head_image_id","teacher_id"])
