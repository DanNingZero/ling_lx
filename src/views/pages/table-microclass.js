...
class App extends React.Component {
    constructor(props) {
        ...
        this.state = {
            head:[
                ...
                },"标题","原价","折扣价",
                {
                    title:"视频",
                    convert:(item,row)=>{
                        return <Select storage="resource" disabled value={item} getJson={sql.find.bind(this,'resource',['id','filename'],"where id>83")}></Select>
                    },
                    renderInput:(row,key,title)=>{
                        return <Select storage="resource" title={title} value={row[key]} onChange={(v)=>row[key]=v} getJson={sql.find.bind(this,'resource',['id','filename'],"where id>83")}></Select>
                    }
                },
                {
                    title:"分类",
                ...
                },
                {
                    title:"头像",
                    convert:(item,row)=>{
                        return <Select storage="resource.image" disabled value={item} getJson={sql.find.bind(this,'resource',['id','filename'],"where id>83 and ext in ('jpg','png')")}></Select>
                    },
                    renderInput:(row,key,title)=>{
                        return <Select storage="resource.image" title={title} value={row[key]} onChange={(v)=>row[key]=v} getJson={sql.find.bind(this,'resource',['id','filename'],"where id>83 and ext in ('jpg','png')")}></Select>
                    }
                },
                {
                    title:"老师",
        ...
        this.sql = new sql("micro_class",["id","title","price","promotion_price","video_id","category_id","head_image_id","teacher_id"])
        this.sql.find("where id>35").then((data)=>{
