...
//在线答题
class Lx_answer2 extends React.Component {
    constructor(props) {
        super(props);
    }
    render() {
...
class TestPaperTab extends React.Component{
    ...
    check_title_index(index){
        return index===this.state.moreng ? "testpaper_box_a on" : "testpaper_box_a";
    }    //Tab切换块方法

    check_item_index(index){
        return index===this.state.moreng ? "oa_Tab_content show" : "oa_Tab_content";
    }    //Tab内容区域方法
    render(){
        return<div>
                {/*Tab切换块*/}
    ...
    render() {
        ...
        return <div className="test_paper_list">
                    ...
                    <div className="test_paper_list_con">
                        {/*{
                            this.props.data.map(function(en,i){
                                return <T_p_l_c_box source={en} key={i} />
                            })
                        }*/}
                    ...
                    <a href="">
                        ...
                        <div className="box_rihgt">
                            <div className="b_left">
                                {/*<h3>{source.name}</h3>
                                <p className="b_many"><em>¥</em><em className="b_nmb">{source.money}</em></p>
                                <span>教材版本：{source.edition}</span> | <span>上传时间：{source.time}</span>*/}
                            ...
                            <div className="b_right">
                                {/*
                                <p className="fengshu"><em className="f_nmb">{source.rate}</em><em>分</em></p>
                                <div className="br_conent"><span>点击：<em>{source.click}</em></span> | <span>购买：<em>{source.buy}</em></span> | <span>收藏：<em>{source.coll}</em></span> </div>
                                */}
